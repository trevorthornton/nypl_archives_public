(function() {
	...
	*/	
	...
	networkAddData: function(data){
		...
		_.each(data.nodes, function(v,i){



			//is it already in the array?
			if (!self.nodeIndex[v.id]){
				...
				var isPrimary = (v.primary) ? true : false;

				//make all ids requested primary

				var orgId = v.id.replace(/c/ig,'').replace(/t/ig,'');

				self.networkNodes.push({id: useId, title : v.title, type: v.type, primary : isPrimary, orgId: orgId, collectionId : v.collectionId });
				self.nodeIndex[v.id] = useId;
				...
				//if (self.lockNodes){
				//	//if it is already in then lock it
				//	_.each(self.networkNodes, function(x,i){
				//		if (x.id == self.nodeIndex[v.id]){
				//			self.networkNodes[self.nodeIndex[v.id]].fixed = true;
				//		}
				//	});
				//}
		...
		_.each(data.edges, function(v,i){
